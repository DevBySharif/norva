...
export async function requireCatalogManager() { const session = await getCurrentUser(); if (!session?.user) redirect("/admin/login"); if (!canManageCatalog(session.user.role)) throw new Error("Forbidden"); return session.user; }
